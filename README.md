...
Почта при обычной регистрации пока не подтверждается письмом; самостоятельное восстановление пароля через email не реализовано. Перед публичным запуском нужны почтовое подтверждение/восстановление, HTTPS с `COOKIE_SECURE=true`, резервное копирование и ограничения нагрузки. Запускайте один процесс сервера: фоновые задания пока выполняются внутри него; после перезапуска незавершённые задания помечаются ошибкой для повторного запуска.
